...
def generate_launch_description() -> LaunchDescription:
    ...
    args = [
        # ROS / sensor
        DeclareLaunchArgument("lidar_topic",      default_value="/lidar/points",    description="PointCloud2 input topic"),
        DeclareLaunchArgument("map_frame",         default_value="map",              description="Map TF frame (unused)"),
        ...
        DeclareLaunchArgument("x_range",    default_value="12.0", description="Grid half-extent in x (m)"),
        DeclareLaunchArgument("y_range",    default_value="12.0", description="Grid half-extent in y (m)"),
        ...
        DeclareLaunchArgument("smooth_sigma",             default_value="0.8",   description="Gaussian smoothing sigma (m)"),

        # Outlier
        DeclareLaunchArgument("outlier_enable",            default_value="true", description="Enable outlier filtering"),
        DeclareLaunchArgument("outlier_type",              default_value="ror",  description="Outlier algorithm: ror | sor"),
        DeclareLaunchArgument("outlier_search_radius_m",   default_value="0.25", description="Neighbor search radius (m)"),
        DeclareLaunchArgument("outlier_min_neighbors",     default_value="10",   description="Min neighbors within radius"),
        DeclareLaunchArgument("outlier_std_multiplier",    default_value="1.0",  description="SOR std multiplier (unused for ROR)"),

        # Traversability
        DeclareLaunchArgument("trav_enable",                     default_value="true", description="Compute traversability cost layers"),
        DeclareLaunchArgument("trav_max_slope_deg",              default_value="60.0", description="Slope saturating cost to 1 (deg)"),
        DeclareLaunchArgument("trav_max_step_height_m",          default_value="0.55", description="Upward step saturating cost to 1 (m)"),
        DeclareLaunchArgument("trav_max_drop_height_m",          default_value="0.3",  description="Downward drop saturating cost to 1 (m)"),
        ...
        # Temporal filter
        DeclareLaunchArgument("filter_enable",                    default_value="true", description="Enable obstacle inflation + temporal gate"),
